...
import { Tag, BarChart2, TrendingUp, CheckCircle } from 'lucide-react';
import { adminAPI } from '../api';
import AdminLayout from '../components/AdminLayout';

const CAT_COLOR: Record<string, string> = {
  politik:'#a371f7', spo:'#3fb950', ekonomi:'#d29922',
  kilti:'#f97316', sosyal:'#58a6ff', lot:'#8b949e'
};
const CAT_EMOJI: Record<string, string> = {
  politik:'🗳️', spo:'⚽', ekonomi:'💰', kilti:'🎭', sosyal:'🏘️', lot:'📌'
};
    ...
    <AdminLayout>
      <div style={{ padding:24 }}>
                ...
                <div key={cat.id} style={{ ...CARD, borderColor:`${color}20` }}>
                  <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:14 }}>
                    <div style={{ width:44, height:44, borderRadius:10, background:`${color}15`,
                      border:`1px solid ${color}30`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:20 }}>
                      {CAT_EMOJI[cat.id] || '📌'}
                    </div>
                    <div>
        ...
        <div style={{ marginTop:24, background:'rgba(31,111,235,0.08)', border:'1px solid rgba(31,111,235,0.2)', borderRadius:10, padding:'12px 16px', fontSize:12, color:'#8b949e', display:'flex', gap:8 }}>
          ...
          <span>Kategori yo defini nan sistèm. Pou ajoute yon nouvo kategori, kontakte devlopè a. Kategori aktyèl yo: <strong style={{ color:'white' }}>Politik, Spò, Ekonomi, Kilti, Sosyal, Lòt</strong>.</span>
        </div>
      </div>
    </AdminLayout>
  );
